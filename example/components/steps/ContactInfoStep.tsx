...
export const ContactInfoStep: React.FC<ContactInfoStepProps> = observer(({ store }) => {
  ...
  const validateField = (name: keyof ContactInfoData, value: string): string | undefined => {
    switch (name) {
      case 'email':
      ...
      case 'phone':
        ...
        if (!/^\d{10}$/.test(value.replace(/\D/g, ''))) return 'Invalid phone number';
        break;
      case 'state':
